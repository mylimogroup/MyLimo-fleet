          ...
          <ul className="divide-y divide-border">
            {recentActivity.map((activity) => (
              <li key={activity.id} className="flex items-start gap-4 px-5 py-4">
                <div
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${activityColors[activity.type]}`}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted">
                    {activity.vehiclePlate && (
                      <span className="font-mono">{activity.vehiclePlate}</span>
                    )}
                    {activity.vehiclePlate && <span>·</span>}
